...
Case and policy uploads follow the same delivery rule: write durable business state and a PostgreSQL job record before placing the small work reference in BullMQ. Redis coordinates claims, locks, retries, and backoff; it is not the job-history database. The worker appends progress to `job_events`, and the header feed filters those rows to the exact enqueueing user. A job paused for a person's decision - today, a policy awaiting its collection - is listed first until it is resolved, each listed job carries its own latest event however old, and the header pins it under "Needs your decision" with a link straight to the policy. Only the platform administrator receives an aggregate view.

Policy processing is a separate worker route: immutable PDF → page extraction/OCR → collection classification, when none was chosen at upload → clause chunks → embeddings → pgvector → cited rule proposals. Classification files a policy only into one of the tenant's existing collections, only on a quotation verified in the document, a confidence at or above `WORKER_COLLECTION_AUTO_FILE_CONFIDENCE`, and a deterministic word-matching reading that independently reaches the same collection; anything else leaves the policy `awaiting_collection` and its job `paused` for an administrator, announced as `policy.collection_decision_required`. `POST /v1/policies/:id/collection` settles it - an existing collection, or a new one minted the way an upload mints one - moves the policy to processing and writes the audit event in one transaction, recording the suggestion the administrator followed or overrode, and resumes the paused job so its timeline reads straight through. Proposals cannot execute until an administrator reviews their original clause, validation result, and four deterministic fixture classes. Activation writes immutable `policy_rules`; future case runs load the active rules for the tenant/domain/date and pin their identifiers and versions in `rule_runs.input_snapshot`.
...
- A policy's collection can be suggested by a model reading that policy. It chooses only among the tenant's existing collections, its quotation must be in the document, and a model's pick files without an administrator only when a deterministic word-matching reading independently reaches the same collection; a model never creates a collection, because that would let a document name its own category.
