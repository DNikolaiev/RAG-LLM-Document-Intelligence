...
CREATE INDEX IF NOT EXISTS workflow_checkpoint_updated_idx
  ON workflow_checkpoints (tenant_id, updated_at DESC);
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    ...
    'policy_chunks','policy_search_chunks','rule_runs','findings','decisions','workflow_checkpoints','audit_events'
  ]
  LOOP
    ...
    EXECUTE format(
      'CREATE POLICY tenant_isolation ON %I USING (tenant_id = current_setting(''app.tenant_id'', true) OR current_setting(''app.platform_admin'', true) = ''true'') WITH CHECK (tenant_id = current_setting(''app.tenant_id'', true) OR current_setting(''app.platform_admin'', true) = ''true'')',
      table_name
    );
  END LOOP;
END $$;
