...
`src/schema.ts` is the typed table source. Run `pnpm db:generate` in this package to write the ordinary table DDL under `migrations/generated`, review it, then run `9999_post_drizzle.sql` after those generated files to add PostgreSQL-specific vector/full-text/partial indexes, RLS, and grants.
