...
7. **Console**: "Let CaseLens classify" as the upload default, and the decision panel on the policy page showing the suggestion, its quotation, and accept / choose / create. Done. The upload select starts on "Let CaseLens classify it", which sends neither collection field; choosing a collection or creating one still overrides it. A waiting policy's page opens with the decision panel (`#collection-decision`, the anchor the notification links to - the page scrolls to it itself, because it only exists once the policy has loaded): the suggestion and its confidence, the rationale, the quotation as a button that turns the PDF to its page, each reason in plain words, and accept, file into an existing collection (defaulting to the suggested or nearest one) or create one. A policy filed automatically says where it went and on which quotation. The existing web proxy already forwarded any `POST` under `/api/policies`, so no route was added.
