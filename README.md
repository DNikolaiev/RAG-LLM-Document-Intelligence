...
A policy that arrives without a collection is classified by the worker once its text is read. A confident match to one of the tenant's existing collections, backed by a quotation found in the document and independently reached by a deterministic word-matching reading, is filed and the uploader is told where it went; anything less - low confidence, an unverified quotation, or a proposed new collection - pauses the policy as `awaiting_collection` for an administrator, and the uploader's notifications pin it under "Needs your decision" with a link to the policy until it is settled. A model never creates a collection on its own. `WORKER_COLLECTION_CLASSIFIER` selects the configured model or a deterministic word-matching classifier. The API accepts an upload that names no collection; the console's upload form still asks for one until its decision screen exists. An administrator settles a waiting policy with `POST /v1/policies/:id/collection` - an existing collection or a new one - which is audited and resumes processing.
